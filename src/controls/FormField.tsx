...
const FormField: React.FC<FormFieldProps> = ({
  ...
  const layoutClasses =
    layout === 'inline' ? 'sm:grid sm:grid-cols-3 sm:items-start sm:gap-6' : 'flex flex-col gap-2';

  const widthClasses = width === 'full' ? 'w-full' : 'w-auto';

  const labelWrapperClasses =
    layout === 'inline' ? 'sm:col-span-1 flex flex-col' : 'flex items-center justify-between gap-2';

  const controlWrapperClasses =
    layout === 'inline' ? 'sm:col-span-2 mt-2 sm:mt-0' : 'flex flex-col gap-2';
