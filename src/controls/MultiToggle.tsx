...
import { getMultiToggleColorTokens } from './Theme';

export type MultiToggleSize = 'sm' | 'md' | 'lg';
export type MultiToggleColor = 'indigo' | 'blue' | 'emerald' | 'amber' | 'rose' | 'slate';
...
const CONTAINER_HORIZONTAL_PADDING = 4; // matches p-1 class on container
...
const MultiToggle: React.FC<MultiToggleProps> = ({
  ...
  const updateIndicatorPosition = useCallback(() => {
    ...
    const baseWidth =
      shouldLockToMaxWidth && maxOptionWidth ? maxOptionWidth : activeButton.offsetWidth;
        ...
        >
          {options.map((option, index) => {
            ...
            return (
              ...
              >
                <span
                  className={classNames('flex min-w-0 items-center justify-center', sizeStyles.gap)}
                >
                  {option.icon && renderIcon(option.icon, size as IconSize, sizeStyles.icon)}
                  {option.label && (
                    <span className={classNames(sizeStyles.label, 'min-w-0')}>{option.label}</span>
                  )}
                </span>
              </div>
            );
            ...
            >
              ...
              {option.label && (!showOnlyActiveLabel || isActive) && (
                <span
                  ...
                  title={
                    shouldTruncate && typeof option.label === 'string' ? option.label : undefined
                  }
                >
                  {option.label}
                </span>
