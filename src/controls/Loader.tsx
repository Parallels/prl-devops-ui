...
import Spinner, { SpinnerColor, SpinnerProps } from './Spinner';
...
const Loader: React.FC<LoaderProps> = ({
  ...
  const containerClass = classNames(
    ...
    overlay &&
    (glass
      ? 'absolute inset-0 z-50 rounded-inherit bg-white/30 p-6 backdrop-blur-sm dark:bg-neutral-900/30'
      : 'absolute inset-0 z-50 rounded-inherit bg-white/85 p-6 backdrop-blur-sm dark:bg-neutral-900/80'),
    className
  );
  ...
  return (
    <div className={containerClass}>
      ...
      {renderBody()}
      {label && (
        <div className={classNames('text-neutral-600 dark:text-neutral-300', resolvedSize.label)}>
          {label}
        </div>
      )}
    </div>
