...
const toneStyles: Record<
  PanelTone,
  { container: string; header: string; chevron: string; content: string }
> = {
  neutral: {
    container:
      'border-slate-200/70 bg-white/90 text-slate-900 shadow-sm dark:border-slate-800/80 dark:bg-slate-900/60 dark:text-slate-100',
  ...
  info: {
    container:
      'border-blue-200/70 bg-blue-50/80 text-blue-900 shadow-sm dark:border-blue-500/40 dark:bg-blue-950/30 dark:text-blue-100',
  ...
  success: {
    container:
      'border-emerald-200/70 bg-emerald-50/80 text-emerald-900 shadow-sm dark:border-emerald-500/40 dark:bg-emerald-950/30 dark:text-emerald-100',
  ...
  warning: {
    container:
      'border-amber-200/70 bg-amber-50/80 text-amber-900 shadow-sm dark:border-amber-500/40 dark:bg-amber-950/30 dark:text-amber-100',
  ...
  danger: {
    container:
      'border-rose-200/70 bg-rose-50/80 text-rose-900 shadow-sm dark:border-rose-500/40 dark:bg-rose-950/30 dark:text-rose-100',
    ...
    >
      ...
      >
        <div className="flex flex-1 flex-col gap-1">
          ...
          {subtitle && (
            <span className={classNames('text-xs text-slate-500 dark:text-slate-300')}>
              {subtitle}
            </span>
          )}
        </div>
        ...
        >
          <CustomIcon icon="ArrowDown" size="sm" />
        </span>
      </button>
      <div
        className={classNames(
          'overflow-hidden transition-[max-height] duration-300',
          isExpanded ? 'mt-3' : 'max-h-0'
        )}
        style={{
