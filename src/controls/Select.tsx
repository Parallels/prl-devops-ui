...
const toneTokens: Record<ButtonColor, SelectToneTokens> = {
  indigo: {
...
const renderVisual = (visual: IconName | React.ReactElement | undefined, className: string) => {
  if (!visual) {
    return null;
  }
...
const Select = forwardRef<HTMLSelectElement, SelectProps>(function Select(
  ...
  const statusClass =
    validationStatus !== 'none' && !unstyled ? statusClasses[validationStatus] : undefined;
  ...
  const caret = useMemo(
    () =>
      showCaret ? (
        ...
        >
          <CustomIcon icon="ArrowDown" className={sizeToken.iconSize} />
        </span>
      ) : null,
    [showCaret, sizeToken.iconRight, sizeToken.iconSize, tokens.icon]
  );
