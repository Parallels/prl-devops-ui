...
import { getTabsColorTokens } from './Theme';
import { iconAccentActive } from './ButtonTypes';
...
const focusRing = (color: ButtonColor) => getTabsColorTokens(color).focusRing;
...
const buildVariantConfig = (
  ...
  switch (variant) {
    ...
    default:
      return {
        container: hideUnderlineContainer
          ? undefined
          : 'border-b border-neutral-500 dark:border-neutral-700',
        list: classNames('gap-6', orientation === 'vertical' && 'gap-3'),
        base: joinClasses(
          'rounded-none font-medium',
          'pb-3 pt-2',
          'border-b-2 border-transparent',
          'after:absolute after:inset-x-1.5 after:bottom-0 after:h-[3px] after:rounded-full after:bg-red-900 after:opacity-0 after:transition-all after:duration-200 dark:after:bg-red-500',
...
const renderIcon = (icon: IconName | React.ReactElement | undefined, iconClasses: string) => {
  if (!icon) {
    return null;
  }

  if (typeof icon === 'string') {
    return <CustomIcon icon={icon} className={iconClasses} />;
  }
...
const Tabs: React.FC<TabsProps> = ({
  ...
  return (
    <div className={rootClass}>
      <div role="tablist" aria-orientation={orientation} className={baseList}>
