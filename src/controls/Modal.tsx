...
import React, { ReactNode, useCallback, useEffect, useId, useMemo, useRef } from 'react';
...
const ModalActions: React.FC<ModalActionsProps> = ({ children, align = 'end', className }) => {
  ...
  return (
    <div
      className={classNames('flex w-full flex-wrap items-center gap-2', alignmentClass, className)}
    >
      {children}
    </div>
  );
...
const Modal: React.FC<ModalProps> = ({
  ...
  }, [closeOnEsc, handleKeyDown, hasDom, hideCloseButton, initialFocusRef, isOpen, preventScroll]);
      ...
      >
        ...
        {tabsConfig && (
          <div className="flex-shrink-0 border-b border-neutral-200/70 px-6 py-2 dark:border-neutral-700/60">
            <Tabs
              {...tabsConfig}
              className={classNames('w-full overflow-x-auto', tabsConfig.className)}
            />
          </div>
        )}

        {bodyHeader && (
          <div className="flex-shrink-0 border-b border-neutral-200/70 bg-neutral-50 px-6 py-3 dark:border-neutral-700/60 dark:bg-neutral-800/60">
            {' '}
            {bodyHeader}
          </div>
        )}
...
interface ConfirmModalProps
  extends Omit<ModalProps, 'footer' | 'actions' | 'children' | 'onClose' | 'title'> {
  title: ReactNode;
  children?: ReactNode;
  onClose: () => void;
  onConfirm: () => void;
