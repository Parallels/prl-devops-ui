...
import { getPanelToneStyles } from './Theme';

export type PanelVariant = 'elevated' | 'outlined' | 'subtle' | 'tonal';
export type PanelTone = 'neutral' | 'info' | 'success' | 'warning' | 'danger' | 'brand';
export type PanelMediaPlacement = 'top' | 'start' | 'end' | 'overlay';
export type PanelPadding = 'xs' | 'sm' | 'md' | 'lg';
...
const variantBaseStyles: Record<PanelVariant, string> = {
  elevated:
    'bg-white/95 text-neutral-900 shadow-lg ring-1 ring-black/5 dark:bg-neutral-900/90 dark:text-neutral-100 dark:ring-white/10',
  outlined:
...
const defaultActionColor: ButtonColor = 'theme';
...
const Panel: React.FC<PanelProps> = ({
  ...
  const titleNode =
    typeof title === 'string' ? (
      <h3 className={classNames('text-xl font-semibold leading-7', headingClass)}>{title}</h3>
    ) : (
      title
    );
  ...
  const subtitleNode =
    typeof subtitle === 'string' ? (
      <p className={classNames('text-base font-medium leading-6', subtitleClass)}>{subtitle}</p>
    ) : (
      subtitle
    );
  ...
  const descriptionNode =
    typeof description === 'string' ? (
      <p className={classNames('text-sm leading-6', descriptionClass)}>{description}</p>
    ) : (
      description
    );
  ...
  const bodySection = bodyContent ? (
    <div
      className={`${flexBody ? 'flex-1 flex flex-col w-full ' : ''}min-h-0 flex-1 overflow-auto pr-1`}
    >
      {bodyContent}
    </div>
  ) : null;
  ...
  const mediaTopNode =
    hasMedia && mediaPlacement === 'top' ? <div className="overflow-hidden">{media}</div> : null;
