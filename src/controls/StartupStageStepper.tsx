...
import type { StartupStage, StartupStageStatus } from '@/interfaces/App';
...
export const StartupStageStepper: React.FC<StartupStageStepperProps> = ({
  ...
  const selectedStage =
    orderedStages.find((stage) => stage.id === selectedStageId) ?? orderedStages[0];
              ...
              {historyCandidates.map((stage) => {
                ...
                return (
                  <li key={stage.id} className="flex items-center justify-between py-2">
                    ...
                    <span className={classNames('text-xs font-semibold uppercase', token.pillText)}>
                      {token.label}
                    </span>
                  </li>
                );
