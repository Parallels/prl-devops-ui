...
function ProxyHostLabel({ host }: { host: ReverseProxyHost }) {
    ...
    const isUnhealthy =
        ...
        (!hasTcp && (host.http_routes ?? []).some(
            (r) => r.target_vm_id && r.target_vm_details?.state?.toLowerCase() === 'stopped'
        ));
    ...
    return (
        <div className="flex flex-col gap-1 min-w-0 flex-1 w-full">
            ...
            <div className="flex items-center gap-1 flex-wrap">
                {hasTcp ? (
                    <Pill size="sm" tone="violet" variant="soft">TCP</Pill>
                ) : httpCount > 0 ? (
                    <Pill size="sm" tone="sky" variant="soft">{httpCount} HTTP {httpCount !== 1 ? 'routes' : 'route'}</Pill>
                ) : (
                    !hasRoutes && <Pill size="sm" tone="neutral" variant="soft">No routes</Pill>
                )}
                {isUnhealthy && (
                    <Pill size="sm" tone="rose" variant="soft">Unhealthy</Pill>
                )}
            </div>
        </div>
    );
...
export const ReverseProxy: React.FC<ReverseProxyProps> = ({ orchestratorHostId }) => {
    ...
    const handleAddHost = useCallback(async (data: Partial<ReverseProxyHost>) => {
        try {
            const created = await devopsService.reverseProxy.createReverseProxyHost(
                hostname, data, orchestratorHostId
            );
    ...
    const handleDeleteHost = useCallback(async (host: ReverseProxyHost) => {
        if (!host.id) return;
        setDeleting(true);
        try {
    ...
    const subtitle = loading
        ? 'Loading…'
        : `${hosts.length} proxy host${hosts.length !== 1 ? 's' : ''}${config?.host ? ` · ${config.host}:${config.port}` : ''}`;
        ...
        <div className="flex flex-col h-full min-h-0 bg-neutral-50 dark:bg-neutral-950">
            ...
            <PageHeader
                ...
                actions={<>
                    ...
                    <div className="flex items-center gap-1.5 pr-1 border-r border-neutral-200 dark:border-neutral-700 mr-1">
                        <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${isEnabled ? 'bg-emerald-500' : 'bg-neutral-400'}`} />
            ...
            >
                <p className="text-sm text-neutral-500 dark:text-neutral-400">
                    This will remove the proxy host and all its configured routes permanently.
                </p>
            </DeleteConfirmModal>
            ...
            >
                <p className="text-sm text-neutral-500 dark:text-neutral-400">
                    {pendingToggle
                        ? 'The reverse proxy engine will start and begin routing traffic.'
                        : 'The reverse proxy engine will stop and all traffic routing will be suspended.'}
                </p>
            </ConfirmModal>
