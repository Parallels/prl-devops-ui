...
const TagInput: React.FC<TagInputProps> = ({ values, placeholder, onChange, disabled, suggestions = [] }) => {
    ...
    return (
        <div className={`rounded-lg border border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-800 p-1.5 min-h-[38px] flex flex-wrap gap-1 ${disabled ? 'opacity-50 pointer-events-none' : ''}`}>
            {values.map((v) => (
                <span key={v} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-medium bg-sky-100 dark:bg-sky-900/40 text-sky-700 dark:text-sky-300">
                    {v}
                    <button type="button" onClick={() => onChange(values.filter((x) => x !== v))} className="hover:text-rose-500">×</button>
                </span>
            ))}
            <input
...
const RouteCard: React.FC<RouteCardProps> = ({ index, route, availableVms, errors, onUpdate, onRemove }) => {
    ...
    const targetSummary = route.targetType === 'vm'
        ? (availableVms.find((v) => v.ID === route.targetVmId)?.Name ?? (route.targetVmId || '—'))
        : (route.targetHost || '—');
    ...
    return (
        <div className="rounded-xl border border-neutral-200 dark:border-neutral-700 overflow-hidden">
            ...
            <div className="flex items-center gap-2 px-3 py-2 bg-neutral-50 dark:bg-neutral-800/50">
                <CustomIcon icon="Script" className="w-4 h-4 text-neutral-400 flex-shrink-0" />
                <span className="font-mono text-sm text-neutral-800 dark:text-neutral-200 truncate flex-1">
                    {route.path || '/'}
                </span>
                <Pill size="sm" tone={route.schema === 'https' ? 'emerald' : 'sky'} variant="soft">
                    {route.schema.toUpperCase()}
                </Pill>
                <span className="text-xs text-neutral-500 truncate max-w-[120px] hidden sm:block">
                        ...
                        <div className="flex-1">
                            <FormField label="Path / Pattern" required>
                                <Input
                                    ...
                                    size="sm"
                                />
                                {errors[`route_${index}_path`] && (
                                    <p className="mt-1 text-xs text-rose-500">{errors[`route_${index}_path`]}</p>
                                )}
                            </FormField>
                        </div>
                        <div className="w-28">
                            <FormField label="Schema">
                                <Select
                                    value={route.schema}
                                    onChange={(e) => upd({ schema: e.target.value as 'http' | 'https' })}
                                    size="sm"
                                >
                            ...
                            <div className="flex-1">
                                <FormField label="Target Host" required>
                                    <Input
                                        ...
                                        size="sm"
                                    />
                                    {errors[`route_${index}_host`] && (
                                        <p className="mt-1 text-xs text-rose-500">{errors[`route_${index}_host`]}</p>
                                    )}
                                </FormField>
                            </div>
                            <div className="w-28">
                                <FormField label="Port" required>
                                    <Input
                                        ...
                                        size="sm"
                                    />
                                    {errors[`route_${index}_port`] && (
                                        <p className="mt-1 text-xs text-rose-500">{errors[`route_${index}_port`]}</p>
                                    )}
                                </FormField>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-3">
                            <div className="flex-1">
                                <FormField label="Virtual Machine" required>
                                    <Select
                                        value={route.targetVmId}
                                        onChange={(e) => upd({ targetVmId: e.target.value })}
                                        validationStatus={errors[`route_${index}_vm`] ? 'error' : 'none'}
                                        size="sm"
                                    >
                                        {vmOptions.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                                    </Select>
                                    {errors[`route_${index}_vm`] && (
                                        <p className="mt-1 text-xs text-rose-500">{errors[`route_${index}_vm`]}</p>
                                    )}
                                </FormField>
                            </div>
                            <div className="w-28">
                        ...
                        <div className="mt-2 space-y-2 pl-4">
                            {route.requestHeaders.map((row, i) => (
                                <div key={i} className="flex gap-2">
                                    <Input size="sm" placeholder="Header name" value={row.key} className="font-mono flex-1"
                                        onChange={(e) => upd({ requestHeaders: route.requestHeaders.map((r, j) => j === i ? { ...r, key: e.target.value } : r) })} />
                                    <Input size="sm" placeholder="Value" value={row.value} className="font-mono flex-1"
                                    ...
                                    <Button variant="ghost" color="rose" size="sm" leadingIcon="Trash"
                                        onClick={() => upd({ requestHeaders: route.requestHeaders.filter((_, j) => j !== i) })} />
                                </div>
                            ))}
                            <Button variant="outline" color="slate" size="sm" leadingIcon="Add"
                                onClick={() => upd({ requestHeaders: [...route.requestHeaders, { key: '', value: '' }] })}>
                                Add Header
                            </Button>
                        </div>
                    </details>
                        ...
                        <div className="mt-2 space-y-2 pl-4">
                            {route.responseHeaders.map((row, i) => (
                                <div key={i} className="flex gap-2">
                                    ...
                                    <Button variant="ghost" color="rose" size="sm" leadingIcon="Trash"
                                        onClick={() => upd({ responseHeaders: route.responseHeaders.filter((_, j) => j !== i) })} />
                                </div>
                            ))}
                            <Button variant="outline" color="slate" size="sm" leadingIcon="Add"
                                onClick={() => upd({ responseHeaders: [...route.responseHeaders, { key: '', value: '' }] })}>
                                Add Header
                            </Button>
                        </div>
                    </details>
...
export const CreateProxyHostModal: React.FC<CreateProxyHostModalProps> = ({
    ...
    useEffect(() => {
        ...
        setPhase('select');
        setStep(0);
        setName(''); setHost(''); setPort(pickSafePort(usedPortSet(existingHosts))); setRouteMode('http');
        setTlsEnabled(false); setTlsCert(''); setTlsKey('');
        setHttpRoutes([blankRoute()]);
        setTcpTargetType('static'); setTcpTargetHost(''); setTcpTargetPort(''); setTcpTargetVmId('');
        setCorsEnabled(false); setCorsOrigins([]); setCorsMethods(['GET', 'POST', 'OPTIONS']);
        setCorsHeaders([]); setCorsExposeHeaders([]); setCorsCredentials(false); setCorsMaxAge('');
        setErrors({});
    }, [isOpen]);
    ...
    const handleSubmit = useCallback(async () => {
        ...
        name, host, port, tlsEnabled, tlsCert, tlsKey, routeMode,
        httpRoutes, corsEnabled, corsOrigins, corsMethods, corsHeaders,
        corsExposeHeaders, corsCredentials, corsMaxAge,
        tcpTargetType, tcpTargetHost, tcpTargetPort, tcpTargetVmId,
        onSubmit, onClose,
    ]);
        ...
        <div className="space-y-4">
            ...
            <FormField label="Listen Port" required>
                <Input
                    placeholder="80"
                    value={port}
                    onChange={(e) => { setPort(e.target.value); setErrors((p) => ({ ...p, port: '' })); }}
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between rounded-lg border border-neutral-200 dark:border-neutral-700 px-3 py-2.5">
                <div>
                    ...
                    <p className="text-xs text-neutral-500 dark:text-neutral-400">
                        Secure connections with a certificate — port will be set to 443
                    </p>
                </div>
            ...
            {tlsEnabled && (
                <div className="space-y-4 pl-3 border-l-2 border-emerald-200 dark:border-emerald-700">
                    ...
                    >
                        <Textarea
                            value={tlsCert}
                            onChange={(e) => { setTlsCert(e.target.value); setErrors((p) => ({ ...p, tlsCert: '' })); }}
                    ...
                    <FormField
                        label="Private Key"
                        description="PEM-encoded private key (BEGIN PRIVATE KEY)"
                        required
                    >
                        <Textarea
                            value={tlsKey}
                            onChange={(e) => { setTlsKey(e.target.value); setErrors((p) => ({ ...p, tlsKey: '' })); }}
            ...
            {!tlsEnabled && (
                <div className="flex items-center gap-2 p-3 rounded-lg bg-neutral-50 dark:bg-neutral-800/50 border border-neutral-200 dark:border-neutral-700">
                    ...
                    <p className="text-xs text-neutral-500 dark:text-neutral-400">
                        TLS is optional. You can skip this step and add certificates later from the Settings tab.
                    </p>
                </div>
            )}
        </div>
                ...
                <div className="space-y-3">
                    {httpRoutes.length === 0 && (
                        <div className="flex items-center gap-2 p-3 rounded-lg bg-neutral-50 dark:bg-neutral-800/50 border border-neutral-200 dark:border-neutral-700">
                            <CustomIcon icon="Info" className="w-4 h-4 text-neutral-400 flex-shrink-0" />
                            <p className="text-xs text-neutral-500 dark:text-neutral-400">
                                No routes yet. Add at least one HTTP route, or add them later from the detail panel.
                            </p>
                        </div>
                    )}
                    ...
                    <Button
                        ...
                        size="sm"
                        leadingIcon="Add"
                        onClick={() => setHttpRoutes((p) => [...p, blankRoute()])}
                    >
                        Add HTTP Route
                    </Button>
                </div>
            ...
            return (
                <div className="space-y-4">
                    <FormField label="Target Type">
                        <MultiToggle
                            value={tcpTargetType}
                            onChange={(v) => { setTcpTargetType(v as TargetType); setErrors({}); }}
                            options={[
                                { value: 'static', label: 'Static IP / Host' },
                                { value: 'vm', label: 'Virtual Machine' },
                            ]}
                            ...
                            <div className="flex-1">
                                <FormField label="Target Host" required>
                                    <Input
                                        placeholder="10.0.0.5 or hostname"
                                        value={tcpTargetHost}
                                        onChange={(e) => { setTcpTargetHost(e.target.value); setErrors((p) => ({ ...p, tcpHost: '' })); }}
                            ...
                            <div className="w-28">
                                <FormField label="Port" required>
                                    <Input
                                        placeholder="5432"
                                        value={tcpTargetPort}
                                        onChange={(e) => { setTcpTargetPort(e.target.value); setErrors((p) => ({ ...p, tcpPort: '' })); }}
                        ...
                        <div className="flex gap-3">
                            <div className="flex-1">
                                <FormField label="Virtual Machine" required>
                                    <Select
                                        value={tcpTargetVmId}
                                        onChange={(e) => { setTcpTargetVmId(e.target.value); setErrors((p) => ({ ...p, tcpVm: '' })); }}
            ...
            {corsEnabled && (
                <div className="space-y-4 pl-3 border-l-2 border-sky-200 dark:border-sky-700">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                            {HTTP_METHODS.map((m) => (
                                <button
                                    key={m}
                                    type="button"
                                    onClick={() => setCorsMethods((prev) =>
                                        prev.includes(m) ? prev.filter((x) => x !== m) : [...prev, m]
                                    )}
                                    className={`px-2.5 py-1 rounded-md text-xs font-semibold transition-colors ${corsMethods.includes(m)
                    ...
                    <FormField label="Expose Headers" description='Headers exposed to the browser.'>
                        <TagInput
                            values={corsExposeHeaders}
                            placeholder="X-Custom-Header"
                            onChange={setCorsExposeHeaders}
                        />
                    </FormField>
        ...
        <div className="flex flex-col gap-5">
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                <button
                    ...
                    className={`group relative text-left rounded-xl border-2 p-4 transition-all duration-200 focus:outline-none ${routeMode === 'http'
                ...
                >
                    ...
                    {/* Icon */}
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-3 transition-colors ${routeMode === 'http'
                        ? 'bg-sky-500 shadow-sm shadow-sky-500/30'
                        : 'bg-neutral-100 dark:bg-neutral-700 group-hover:bg-sky-100 dark:group-hover:bg-sky-900/40'
                        }`}>
                        <CustomIcon icon="ReverseProxyRoutes" className={`w-5 h-5 ${routeMode === 'http' ? 'text-white' : 'text-neutral-500 dark:text-neutral-400 group-hover:text-sky-600 dark:group-hover:text-sky-400'}`} />
                    </div>

                    <p className={`text-sm font-semibold mb-1 ${routeMode === 'http' ? 'text-sky-700 dark:text-sky-300' : 'text-neutral-800 dark:text-neutral-200'}`}>
                        HTTP Routes
                    </p>
                    <p className="text-xs text-neutral-500 dark:text-neutral-400 leading-relaxed mb-3">
                        Path-based routing, TLS termination, CORS and custom headers.
                    </p>
                    ...
                    <ul className="space-y-1">
                        {['Web apps & APIs', 'WebSocket proxying', 'CORS & TLS control'].map((f) => (
                            <li key={f} className="flex items-center gap-1.5 text-xs text-neutral-500 dark:text-neutral-400">
                                <span className={`w-1 h-1 rounded-full flex-shrink-0 ${routeMode === 'http' ? 'bg-sky-400' : 'bg-neutral-300 dark:bg-neutral-600'}`} />
                                {f}
                            </li>
                        ))}
                    </ul>
                ...
                <button
                    ...
                    className={`group relative text-left rounded-xl border-2 p-4 transition-all duration-200 focus:outline-none ${routeMode === 'tcp'
                        ? 'border-violet-500 bg-violet-50 dark:bg-violet-950/30 shadow-md shadow-violet-500/10'
                        : 'border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800/60 hover:border-violet-300 dark:hover:border-violet-700 hover:bg-violet-50/50 dark:hover:bg-violet-950/20'
                        }`}
                >
                    {/* Selected check */}
                    {routeMode === 'tcp' && (
                    ...
                    {/* Icon */}
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-3 transition-colors ${routeMode === 'tcp'
                        ? 'bg-violet-500 shadow-sm shadow-violet-500/30'
                        : 'bg-neutral-100 dark:bg-neutral-700 group-hover:bg-violet-100 dark:group-hover:bg-violet-900/40'
                        }`}>
                        <CustomIcon icon="Live" className={`w-5 h-5 ${routeMode === 'tcp' ? 'text-white' : 'text-neutral-500 dark:text-neutral-400 group-hover:text-violet-600 dark:group-hover:text-violet-400'}`} />
                    </div>

                    <p className={`text-sm font-semibold mb-1 ${routeMode === 'tcp' ? 'text-violet-700 dark:text-violet-300' : 'text-neutral-800 dark:text-neutral-200'}`}>
                        TCP Route
                    </p>
                    <p className="text-xs text-neutral-500 dark:text-neutral-400 leading-relaxed mb-3">
                        Raw TCP tunnel to a single backend — ideal for non-HTTP protocols.
                    </p>

                    <ul className="space-y-1">
                        {['Databases (Postgres, MySQL)', 'Redis / message queues', 'SSH & custom protocols'].map((f) => (
                            <li key={f} className="flex items-center gap-1.5 text-xs text-neutral-500 dark:text-neutral-400">
                                <span className={`w-1 h-1 rounded-full flex-shrink-0 ${routeMode === 'tcp' ? 'bg-violet-400' : 'bg-neutral-300 dark:bg-neutral-600'}`} />
                                {f}
                            </li>
                        ))}
                    </ul>
            ...
            <div className="flex items-center gap-1.5 px-1">
                {(routeMode === 'http'
                    ? ['General', 'TLS', 'Routes', 'CORS']
                    : ['General', 'TLS', 'Route']
                ).map((label, i, arr) => (
                    <React.Fragment key={label}>
                        <div className="flex items-center gap-1">
                            <span className={`w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center flex-shrink-0 ${routeMode === 'http' ? 'bg-sky-100 dark:bg-sky-900/40 text-sky-600 dark:text-sky-400' : 'bg-violet-100 dark:bg-violet-900/40 text-violet-600 dark:text-violet-400'
                                }`}>
                                {i + 1}
                            </span>
                        ...
                        {i < arr.length - 1 && (
                            <div className={`flex-1 h-px min-w-[12px] ${routeMode === 'http' ? 'bg-sky-200 dark:bg-sky-800/60' : 'bg-violet-200 dark:bg-violet-800/60'}`} />
                        )}
                    </React.Fragment>
                ))}
            </div>
    ...
    const stepDefs: Step[] = [
        {
            id: 'general',
            title: <div className='flex items-center gap-2'> <CustomIcon icon="Settings" className="w-4 h-4" />General</div>,
            subtitle: 'Host & port',
        ...
        {
            id: 'tls',
            title: <div className='flex items-center gap-2'> <CustomIcon icon="ReverseProxyTLS" className="w-4 h-4" />TLS</div>,
        ...
        {
            id: 'routes',
            title: <div className='flex items-center gap-2'> <CustomIcon icon="ReverseProxyRoutes" className="w-4 h-4" />Routes</div>,
        ...
        ...(routeMode === 'http' ? [{
            id: 'cors',
            title: <div className='flex items-center gap-2'> <CustomIcon icon="ReverseProxyCORS" className="w-4 h-4" />CORS</div>,
                ...
                <>
                    ...
                    <ModalActions align="between">
                        ...
                        <div className="flex items-center gap-2">
                            <span className="text-xs text-neutral-400">{step + 1} / {totalSteps}</span>
                            {isLastStep ? (
                                <Button
                                    variant="solid"
                                    color={themeColor}
...
export const HttpRouteModal: React.FC<HttpRouteModalProps> = ({
    isOpen,
    editing,
    availableVms,
    onClose,
    ...
    const handleSubmit = useCallback(async () => {
        ...
        setSaving(true);
        try {
            await onSubmit({ path: path.trim(), schema, pattern: pattern.trim(), targetType, targetHost: targetHost.trim(), targetPort: targetPort.trim(), targetVmId, requestHeaders: requestHeaderRows.filter((r) => r.key.trim()), responseHeaders: headerRows.filter((r) => r.key.trim()) });
            onClose();
        } finally {
            setSaving(false);
        }
                ...
                <div className="flex gap-3">
                    ...
                    <div className="w-28">
                        <FormField label="Schema">
                            <Select
                                value={schema}
                                onChange={(e) => setSchema(e.target.value as 'http' | 'https')}
                            >
                                <option value="http">HTTP</option>
                                <option value="https">HTTPS</option>
                            </Select>
                        ...
                        {requestHeaderRows.map((row, i) => (
                            <div key={i} className="flex gap-2">
                                <Input placeholder="Header name" value={row.key}
                                    ...
                                    className="font-mono flex-1" size="sm" />
                                <Input placeholder="Value" value={row.value}
                                    onChange={(e) => setRequestHeaderRows((p) => p.map((r, j) => j === i ? { ...r, value: e.target.value } : r))}
                                    className="font-mono flex-1" size="sm" />
                                <Button variant="ghost" color="rose" size="sm" leadingIcon="Trash"
                                    onClick={() => setRequestHeaderRows((p) => p.filter((_, j) => j !== i))} />
                            </div>
                        ))}
                        <Button variant="outline" color="slate" size="sm" leadingIcon="Add"
                            onClick={() => setRequestHeaderRows((p) => [...p, { key: '', value: '' }])}>
                            Add Header
                        </Button>
                    </div>
                </FormField>
                        ...
                        {headerRows.map((row, i) => (
                            <div key={i} className="flex gap-2">
                                ...
                                <Input placeholder="Value" value={row.value}
                                    ...
                                    className="font-mono flex-1" size="sm" />
                                <Button variant="ghost" color="rose" size="sm" leadingIcon="Trash"
                                    onClick={() => setHeaderRows((p) => p.filter((_, j) => j !== i))} />
                            </div>
                        ))}
                        <Button variant="outline" color="slate" size="sm" leadingIcon="Add"
                            onClick={() => setHeaderRows((p) => [...p, { key: '', value: '' }])}>
                            Add Header
                        </Button>
                    </div>
                </FormField>
