...
function ActionButton({ icon, label, loading, loadingLabel, onClick, disabled, tone }: {
    ...
    return (
        <button
            ...
            className={classNames(
                'flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium transition select-none',
                colors[tone],
...
function HardwareTab({ vm }: { vm: VirtualMachine }) {
    if (!vm.Hardware) {
        return (
            <p className="px-4 py-8 text-center text-xs text-neutral-400 dark:text-neutral-600">
...
export function VmDetailContent({ vm, hostname, isOrchestrator }: VmDetailContentProps) {
    ...
    const handleAction = useCallback(async (action: string) => {
        ...
        try {
            switch (action) {
                case 'start': await devopsService.machines.startVirtualMachine(hostname, vm.ID, isOrchestrator); break;
                case 'stop': await devopsService.machines.stopVirtualMachine(hostname, vm.ID, true, isOrchestrator); break;
                case 'pause': await devopsService.machines.pauseVirtualMachine(hostname, vm.ID, isOrchestrator); break;
                case 'resume': await devopsService.machines.resumeVirtualMachine(hostname, vm.ID, isOrchestrator); break;
                case 'suspend': await devopsService.machines.suspendVirtualMachine(hostname, vm.ID, isOrchestrator); break;
                case 'restart':
    ...
    return (
        <div className="flex flex-col h-full min-h-0">
            {/* ── Hero ──────────────────────────────────────────────── */}
            <div className="flex-shrink-0 px-4 py-4 space-y-3 border-b border-neutral-100 dark:border-neutral-800">
                <div className="flex items-start gap-3">
                    <div className="w-10 h-10 flex-shrink-0 rounded-xl bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center text-neutral-600 dark:text-neutral-300">
                    ...
                    <div className="flex-1 min-w-0 space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {vm.OS && (
                                <span className="text-xs text-neutral-500 dark:text-neutral-400">{vm.OS}</span>
                            )}
                        </div>
                        {vm.Uptime && (
                            <p className="text-xs text-neutral-400 dark:text-neutral-500">Uptime: {formatDuration(parseInt(vm.Uptime))}</p>
                        )}
                        {vm.GuestTools?.version && (
                            <p className="text-xs text-neutral-400 dark:text-neutral-500">Guest tools: <span className={vm.GuestTools.state === 'possibly_installed' ? 'text-sky-400  dark:text-sky-500' : vm.GuestTools.state === 'outdated' ? 'text-amber-400 dark:text-amber-500' : 'text-emerald-400 dark:text-emerald-500'}>{vm.GuestTools.version}</span></p>
                        )}
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                    {isStopped && (
                        <ActionButton icon={<Run className="w-3.5 h-3.5" />}
                            label="Start" loadingLabel="Starting…"
                            loading={actionLoading === 'start'} disabled={busy} tone="emerald"
                            onClick={() => void handleAction('start')} />
                    )}
                    {canResume && (
                        <ActionButton icon={<Run className="w-3.5 h-3.5" />}
                            label="Resume" loadingLabel="Resuming…"
                            loading={actionLoading === 'resume'} disabled={busy} tone="emerald"
                            onClick={() => void handleAction('resume')} />
                    )}
                    {(isRunning || canResume) && (
                        <ActionButton icon={<Stop className="w-3.5 h-3.5" />}
                            label="Stop" loadingLabel="Stopping…"
                            loading={actionLoading === 'stop'} disabled={busy} tone="rose"
                            onClick={() => void handleAction('stop')} />
                    )}
                    {isRunning && (
                        <>
                            <ActionButton icon={<Pause className="w-3.5 h-3.5" />}
                                label="Pause" loadingLabel="Pausing…"
                                loading={actionLoading === 'pause'} disabled={busy} tone="amber"
                                onClick={() => void handleAction('pause')} />
                            <ActionButton icon={<Suspend className="w-3.5 h-3.5" />}
                                label="Suspend" loadingLabel="Suspending…"
                                loading={actionLoading === 'suspend'} disabled={busy} tone="sky"
                                onClick={() => void handleAction('suspend')} />
                            <ActionButton icon={<Restart className="w-3.5 h-3.5" />}
                                label="Restart" loadingLabel="Restarting…"
                                loading={actionLoading === 'restart'} disabled={busy} tone="neutral"
