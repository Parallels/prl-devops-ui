...
command -v rustc >/dev/null 2>&1 || error "Rust not found. Install via rustup."
command -v cargo >/dev/null 2>&1 || error "Cargo not found."
[[ -f "src-tauri/tauri.conf.json" ]] || error "tauri.conf.json not found. Are you in the right project?"
...
seed_memory "project/stack" \
  "Tauri v2 desktop app, React/TypeScript frontend in src/, Rust backend in src-tauri/, IPC via invoke and events" \
  "prl-devops-ui" "procedural"

seed_memory "project/build" \
  "npm run build | npm test | npm run lint | cd packages/ui-kit && npm run build && npm run lint | npm run tauri build | cd src-tauri && cargo clippy" \
  "prl-devops-ui" "procedural"

seed_memory "project/patterns" \
  "IPC boundary must stay clean. Use invoke for request/response, Tauri events for Rust->frontend push. Validate input on both sides. Check tauri.conf.json before changing capabilities." \
  "prl-devops-ui" "procedural"

seed_memory "project/packages" \
  "src/ (React components, hooks, TypeScript), src-tauri/ (Rust commands, plugins), packages/ui-kit/ (internal React component library, own build+lint pipeline)" \
  "prl-devops-ui" "procedural"
