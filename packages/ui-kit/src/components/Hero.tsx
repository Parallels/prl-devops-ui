...
export type HeroPadding      = "sm" | "md" | "lg";
  ...
  return (
    <div
      className={classNames(
        "relative overflow-hidden flex items-center gap-4 bg-gradient-to-br shadow-lg",
        toneGradient[tone],
        paddingMap[padding],
        rounded && "rounded-xl",
        className,
      ...
      {...rest}
    >
      {/* Decoration: floating circles at white/5 opacity — same intensity on all tones */}
      {showShapes && (
        <>
          <div className="pointer-events-none absolute -right-10 -top-10 w-52 h-52 rounded-full bg-white/5" aria-hidden="true" />
          <div className="pointer-events-none absolute -left-8 -bottom-10 w-36 h-36 rounded-full bg-white/5 opacity-70" aria-hidden="true" />
          <div className="pointer-events-none absolute right-10 bottom-8 w-16 h-16 rounded-full bg-white/5 opacity-50" aria-hidden="true" />
        </>
      )}
      ...
      {/* Icon container */}
      {iconNode && (
        <div className="relative flex-shrink-0 w-12 h-12 rounded-xl bg-white/15 backdrop-blur-sm flex items-center justify-center border border-white/20 shadow-inner">
          {iconNode}
        </div>
      )}
      ...
      <div className="relative min-w-0 flex-1">
        <p className={classNames("text-white leading-tight", titleSizeMap[titleSize])}>
          {title}
        </p>
        {subtitle && (
          <p className={classNames("text-white/75 mt-0.5 leading-relaxed", subtitleSizeMap[subtitleSize])}>
            {subtitle}
          </p>
        )}
      </div>
