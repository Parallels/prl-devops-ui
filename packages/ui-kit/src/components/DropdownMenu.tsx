...
export const DropdownMenu: React.FC<DropdownMenuProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    const computedWidth =
      typeof width === "number" ? width : width === "trigger" ? Math.max(anchorRect.width, menuRect.width) : menuRect.width;
    ...
    if (side === "top") {
      ...
      const fitsBelow = anchorRect.bottom + offset + menuRect.height <= viewportHeight - minMargin;
  ...
  return createPortal(
    ...
    >
      <ul className="overflow-auto" style={{ maxHeight }} onClick={(event) => event.stopPropagation()}>
        {items.map((item) => (
          <li key={item.value}>
            <button
              type="button"
