...
const Alert = React.forwardRef<HTMLDivElement, AlertProps>(({ tone, color, variant = 'subtle', title, description, icon, actions, dismissible = false, onDismiss, className, ...rest }, ref) => {
  ...
  return (
    <div ref={ref} className={base} role="alert" {...rest}>
      {resolvedIcon && <div className={classNames('flex-shrink-0 pt-1', tokens.icon)}>{renderIcon(resolvedIcon, 'md')}</div>}
