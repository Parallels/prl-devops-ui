...
const KeyValueArrayField: React.FC<KeyValueArrayFieldProps> = ({ label, hint, value, onChange, error, help, isVisible = true, addLabel }) => {
  ...
  const handlePairChange = (index: number, field: "key" | "value", fieldValue: string) => {
    const nextPairs = pairs.map((pair, i) => (i === index ? { ...pair, [field]: fieldValue } : pair));
    setPairs(nextPairs);
    onChange(nextPairs);
  };
      ...
      <div className="space-y-3">
        {pairs.map((pair, index) => (
          <div key={`${pair.key}-${index}`} className="flex flex-wrap gap-3">
            <Input className="flex-1 min-w-[140px]" size="sm" placeholder="Key" value={pair.key} onChange={(event) => handlePairChange(index, "key", event.target.value)} />
            <Input className="flex-1 min-w-[140px]" size="sm" placeholder="Value" value={pair.value} onChange={(event) => handlePairChange(index, "value", event.target.value)} />
            <Button variant="soft" color="rose" size="sm" onClick={() => handleRemovePair(index)} className="flex-shrink-0">
              Remove
            </Button>
          </div>
        ))}
